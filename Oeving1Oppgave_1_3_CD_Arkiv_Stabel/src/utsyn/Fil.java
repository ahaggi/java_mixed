...
public class Fil {
	...
	public CDarkivADT lesFraFil(String filnavn) throws java.io.IOException {
		...
		try {
			...
			//	if (testInnput(post)) { //først tester inndata som står på filen, etterpå opprette CD objekt
					String[] felt = post.split(SKILLE);
					int nr = Integer.parseInt(felt[0]);
					String artist = felt[1];
					String tittel = felt[2];
					int هr = Integer.parseInt(felt[3]);
					Sjanger sjanger = Sjanger.finnSjanger(Integer.valueOf(felt[4])); //Eventuelt lagret som heltall...Mه sه konvertere til enum
					String plselskap = felt[5];

					CD cd = new CD(nr, artist, tittel, هr, sjanger, plselskap);
					cda.leggTilCd(cd);    
			//	}
...
//			int هr = Integer.parseInt(testFelt[3]);
//			Sjanger sjanger = Sjanger.finnSjanger(Integer.valueOf(testFelt[4])); 
//			String plselskap = testFelt[5];
//			riktig= true;
//			
//		} catch (Exception e) {
//			System.out.println("Feil melding: fّlgende CD ble ikke registerert\n"+ test);
//			riktig = false;
//		}finally {
//			return riktig;
//		}
