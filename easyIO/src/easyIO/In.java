 ...
 * "Rett p� Java".2.utg. 2005 og 2007. Versjon 5 med forbedret versjon
 ...
 * <code>hasNextInt()</code> for � sjekke om neste gruppe av tegn kan
 * tolkes som et heltall, og dermed leses med metoden inInt uten at
 * det oppst�r feil.
 *
 * <p>N�r man gj�r innlesing kan det opps� feil. Det kan for eksempel
 ...
 * oppst�r avsluttes hele programmet, og det gis en melding om �rsaken.
 *
 * <p> En annen type feil som kan oppst�, er at man for eksempel
 * �nsker � lese inn et heltall, men det man faktisk leser er en annen
 * type (for eksempel en tekst). Hvis man leser fra fil vil programmet
 * ogs� da avsluttes med en melding om hva som gikk galt, og hvilken
 * linje i fila det gikk galt p�. Hvis man leser fra tastatur bes
 * brukeren om � skrive inn tallet en gang til. Brukeren gis to
 * muligheter for � skrive riktig tall. Etter det avsluttes programmet
 * hvis brukeren ikke gir riktig input.
 *
 * <h2>Eksempel p� lesing fra fil:</h2>
 *
 * <pre>
 ...
 * </pre>
 *
 * <h2>Eksempel p� lesing fra tastatur:</h2>
 *
 * <pre>
 * // �pner en leser
 * In in = new ();
 ...
 * (<code>setDelimiter</code>) vil p�virke alle
 ...
 *
 * N�r brukeren skriver inn en tekst i kommandovinduet, sendes denne
 * teksten til et <em>in-buffer</em> n�r [ENTER]-tasten trykkes. I
 * dette bufferet ligger da alle tegn som brukeren har skrevet
 * inkludert linjeskifttegnet. S� lenge man holder seg til en
 * lesemodus (tegn for tegn, tegnrupper eller linjer) f�rer dette
 * sjeldent til problemer, men hvis man blander lesemodi m� man ta
 * hensyn til linjeskifttegnet (ett i Unix/Linux, to i DOS og Macintosh).
 *
 * Forskjellen p� <code>inLine() og readLine()</code>
 *
 * <p>Tenk at in-bufferet p� et tidspunkt inneholder f�lgende uleste
 * tegn: <code>'1' '0' '0' '\n'</code>, det vil si at brukeren har
 * skrevet inn "100" og trykket [ENTER]. Hvis vi n� leser inn tallet
 * med metoden <code>inInt</code> leses de tre f�rste tegnene, som
 * s� konverteres til en <code>int</code> med verdien 100. In-bufferet
 * inneholder da et eller to ulest tegn, nemlig '\n' (evt \n\r), som er tegn for
 * linjeskift. Hvis vi s� �nsker at brukeren skal skrive inn en linje
 ...
 * til og med f�rste linjeskifttegn, og returnerer teksten untatt
 * linjeskifttegn. Man m� derfor f�rst kvitte seg med
 * linjeskifttegn, ved for eksempel � kalle p� metoden
 * <code>skipWhite</code>.
 *
 * For � b�te p� dette problemet tilbyr klassen metoden
 * <code>inLine</code>. Denne metoden leser f�rst alle tegnene forbi
 * f�rste linjeskift-tegn, og returnerer s� teksten frem til neste
...
public class In extends InExp {
    ...
    /**
     * Setter <a href="#skilletegn">skilletegn</a> til forh�ndsvalgt
    ...
    * f�rste tegnet som ikke h�rer til blant <a href="#skilletegn">skilletegn.</a>, hvis parameteren er 'false'
    ...
    * returnerer det f�rste uleste tegnet (uansett om det er skilletegn eller ikke).
    * @return tegnet som ble lest.
    * @see BufferedReader#read
    */
    public char inChar() {
     ...
     * href="#skilletegn">skilletegn</a> (evt. slutt p� fil/input)
     * @since versjon 2
     */
    public void skipSep() {
    try {
     ...
     *<code>sep</code> eller de blanke tegnene som alltid er skilletegn.  (evt. slutt p� fil/input).
     * @param sep teksten med skilletegn.
     */
    ...
    */
    ...
    for (int i = 0; i < numTry(); i++) {
        ...
        if (FILE) {
            ...
            System.out.print("Forventer en byte. Pr�v igjen: ");
        }
        }
    }
    ...
    for (int i = 0; i < numTry(); i++) {
        ...
        if (FILE) {
            ...
            System.out.print("Forventer et desimaltall. Pr�v igjen: ");
        }
        }
    }
    ...
    public int inInt() {
      String cause = null;
        ...
        if (FILE) {
            ...
            System.out.print("Forventer et heltall. Pr�v igjen: ");
        }
        }
    }
     ...
     * @return <code>true</code> hvis det finnes tegn � lese.
     */
